...
export function ProjectCard({ project }: ProjectCardProps) {
  return (
    <article className="group border-t border-(--color-rule-storm) py-5 last:border-b">
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-start">
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-xl font-semibold tracking-normal text-(--color-ink)">
            <a className="underline decoration-(--color-rule-rain) underline-offset-4 transition-colors duration-(--dur-fast) group-hover:text-(--color-graphite)" href={`/projects/${project.slug}`}>
              {project.title}
            </a>
          </h2>
